...
TestSuite CreateProceduralSuite()
{
    ...
    AddTest(suite, "grid points are centered around origin",
            []
            {
                ...
                AssertVec3Near(points.front().Position, glm::vec3(-2.0f, 0.0f, -3.0f), 1e-5f,
                               "Grid should start at the negative half extents");
    ...
    AddTest(suite, "transform points updates heights",
            []
            {
                ...
                grid.init(2.0f, 2.0f, 3, 3);
                grid.GenerateTriangles();
                grid.TransformPoints([](Vertex &vertex, unsigned int index) { vertex.Position.y = static_cast<float>(index); });
                const std::vector<Vertex> points = grid.GetPoints();
                ...
                for (const Vertex &vertex : points)
                {
                    Assert(std::isfinite(vertex.Normal.x) && std::isfinite(vertex.Normal.y) && std::isfinite(vertex.Normal.z),
                           "Normals should remain finite after transforming points");
                }
            });
    ...
    AddTest(suite, "grid copy preserves generated data",
            []
            {
                ...
                AssertVec3Near(copy.GetPoints()[3].Position, original.GetPoints()[3].Position, 1e-6f,
                               "Copied grid should preserve point positions");
            });
