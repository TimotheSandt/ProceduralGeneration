...
double LengthSquared2(float x, float y) {
...
TestSuite CreateUtilitiesSuite() {
    ...
    AddTest(suite, "lerp returns midpoint", [] {
        AssertNear(lerp(10.0f, 20.0f, 0.5f), 15.0, 1e-6, "lerp should compute the midpoint");
    });
    ...
    AddTest(suite, "executable path is available", [] {
        Assert(!GetExecutablePath().empty(), "Executable path should not be empty");
    });
    ...
    AddTest(suite, "executable directory matches executable path", [] {
        ...
        AssertEqual(GetExecutableDirectory(), executablePath.parent_path().string(), "Executable directory should match the executable path parent");
    });
    ...
    AddTest(suite, "set working directory moves to executable directory", [] {
        ...
        AssertEqual(fs::current_path().string(), GetExecutableDirectory(), "Working directory should become the executable directory");
