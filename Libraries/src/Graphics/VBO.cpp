...
VBO::VBO(VBO &&other) noexcept : ID(0) { this->Swap(other); }
...
VBO::VBO(std::vector<GLfloat> &vertices) { Initialize(vertices); }

VBO::~VBO() { this->Destroy(); }

void VBO::Swap(VBO &other) noexcept { std::swap(this->ID, other.ID); }
