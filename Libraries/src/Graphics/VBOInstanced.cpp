...
VBOInstanced::VBOInstanced(std::vector<glm::mat4> &mat4) : VBO() { Initialize(mat4); }

VBOInstanced::VBOInstanced(VBOInstanced &&other) noexcept { this->Swap(other); }
...
void VBOInstanced::UploadData(const void *data, GLsizeiptr size) const { this->UploadData(data, size, 0); }
