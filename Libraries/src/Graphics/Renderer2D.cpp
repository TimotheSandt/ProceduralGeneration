...
void Renderer2D::BeginFrame(int width, int height)
{
    frameWidth = width;
    frameHeight = height;
...
void Renderer2D::BeginTextPass() const
{
...
void Renderer2D::EndTextPass() const
{
    if (!IsRuntimeCompatible())
    {
        return;
...
void Renderer2D::EnableClipRect(float x, float y, float width, float height) const
{
    if (!IsRuntimeCompatible())
    {
        return;
...
void Renderer2D::DisableClipRect() const
{
    if (!IsRuntimeCompatible())
    {
        return;
                                    ...
                                    const UI::TextLayoutParams &params, const glm::vec3 &color, float scale) const
{
    ...
    if (useScissor)
    {
        EnableClipRect(x, y, params.maxWidth, params.maxHeight);
    }
