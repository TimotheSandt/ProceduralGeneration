...
EBO::EBO(std::vector<GLuint> &indices) { Initialize(indices); }

EBO::EBO(EBO &&other) noexcept : ID(0) { std::swap(this->ID, other.ID); }
...
void EBO::Swap(EBO &other) noexcept { std::swap(this->ID, other.ID); }
