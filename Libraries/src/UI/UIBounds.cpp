...
{
    if ((parentAvailableSize.x <= 0 || parentAvailableSize.y <= 0) &&
        (width.type == ValueType::PERCENT || height.type == ValueType::PERCENT))
    {
    ...
    scale = {getValueInPixel(width, static_cast<int>(parentAvailableSize.x)),
...
bool Bounds::isHover(const glm::vec2 &mousePos) const
{
    ...
    return mousePos.x >= 0 && mousePos.x <= scale.x && mousePos.y >= 0 && mousePos.y <= scale.y;
}
