...
template <typename T> bool IsEqual(const std::weak_ptr<T> &a, const std::weak_ptr<T> &b)
{
    return !a.owner_before(b) && !b.owner_before(a);
}
} // namespace DeferredValueDetail

template <typename T> class DeferredValue
{
  public:
    ...
    template <typename Func> void ModifyForce(Func func)
    {
        T temp = value;
        func(temp);
        ForceSet(temp);
