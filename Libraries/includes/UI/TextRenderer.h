...
class TextRenderer
{
  public:
    ...
    bool loadFont(const std::string &fontPath, const std::string &fontName, unsigned int fontSize = 48);
    ...
    void renderTextAdvanced(const std::string &text, float x, float y, const TextLayoutParams &params,
    ...
    TextLayout calculateLayout(const std::string &text, float scale, const TextLayoutParams &params);

    // Calcul taille minimale pour contenir le texte
    glm::vec2 calculateMinSize(const std::string &text, float scale = 1.0f, bool wordWrap = false);

    // === HELPERS POUR UI ===
    // Centrer texte dans un rectangle
    glm::vec2 getCenteredPosition(const std::string &text, float scale, float rectX, float rectY, float rectWidth, float rectHeight);

    // Trouver la taille de police optimale pour un rectangle
    float findOptimalScale(const std::string &text, float maxWidth, float maxHeight, float minScale = 0.3f, float maxScale = 2.0f);
